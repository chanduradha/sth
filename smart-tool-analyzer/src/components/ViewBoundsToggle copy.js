...
const ViewBoundsToggle = ({ chartRef, leftLinePosition, rightLinePosition, chartData, setChartData, originalChartData, setOriginalChartData }) => {
  ...
  const handleBoundsSelection = (option) => {
    ...
    applyDriftCompensation(); // New function for drift compensation
  }
  };
  
  const clearOffset = () => {
    setChartData(originalChartData); // Reset chart data to original data
  };
...
const cutData = () => {
  ...
  // Filter out the data points within the range from the originalChartData
  ...
  setChartData(newData);
  setOriginalChartData(filteredOriginalData); // Update originalChartData with filtered data
};


const applyDriftCompensation = () => {
  // Identify reference points (e.g., start and end values)
  ...
  const drift = {
    ...
    // Calculate drift for other properties as needed
  };
  ...
  const compensatedData = chartData.map((item) => ({
    ...
    // Apply correction for other properties
  }));

  // Update state with compensated data
  ...
  // const handleMouseUp = () => {
  //   // Update line positions when dragging is completed
  ...
  const calculateValues = () => {
    ...
    console.log('Data within the range:', filteredData);
