...
const LineGraph = ({ data, isOtherWindowOpen, importedFileName, selectedFiles: propSelectedFiles ,selectedFilesNew  }) => {
  ...
  const [originalChartData, setOriginalChartData] = useState(data); 
...
function calculateRollingMedian(data, windowSize) {
    ...
    const addValueToWindow = (value) => {
        // Add value to window while maintaining sorted order using binary search
        let left = 0;
        ...
        window.splice(left, 0, value);
    };

    const removeValueFromWindow = (value) => {
        // Remove value from window
        const index = window.indexOf(value);
        if (index !== -1) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        addValueToWindow(data[i]);

        // If window size exceeds, remove the oldest value
        if (window.length > windowSize) {
            removeValueFromWindow(data[i - windowSize]);
        }
        ...
        if (window.length % 2 === 0) {
            // Even-sized window
            const median = (window[mid - 1] + window[mid]) / 2;
            rollingMedian.push(median);
        } else {
            // Odd-sized window
            rollingMedian.push(window[mid]);
        }
    }
...
>
  {averagingEnabled ? 'Averaging Off' : 'Averaging On'}
      ...
      data={data} // Pass setOriginalChartData here
    /> 
   </div>
  <button onClick={handleOpenPopup} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded absolute top-0 right-0 mt-4 mr-4  focus:outline-none focus:ring-2 focus:ring-offset-2" style={{position:'absolute',top:'900px',right:'140px'}}>
  Open Popup
</button>
<div>
{selectedFilesNew.length > 0 && <SaveFiles selectedFiles={selectedFilesNew}  />}
</div>
<Popup isOpen={isPopupOpen} onClose={handleClosePopup}  selectedFiles={selectedFilesNew}/> 
